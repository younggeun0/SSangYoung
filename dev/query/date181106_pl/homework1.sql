...
DECLARE                             	
	...
	idx NUMBER := 0;
 	...
 	-- 메이커1,메이커2의 차량 수를 담을 변수
 	cnt_maker1 NUMBER := 0;
 	cnt_maker2 NUMBER := 0;

BEGIN              
  
  -- 데이터 처리(커서를 이용 임시 레코드에 값을 가져와서 테이블에 추가)
	FOR rec_temp IN 
		(SELECT country, maker, model, car_year, price, hiredate	 
		 FROM(SELECT ROWNUM r, country, maker, model, car_year, price, hiredate
			    FROM(SELECT cc.country, cma.maker, cmo.model, cmo.car_year, 
			    						cmo.price, cmo.hiredate
	...
	IF arr_car.COUNT != 0 THEN      
		...
		DBMS_OUTPUT.PUT_LINE('---------------------------------------------------------');
	  DBMS_OUTPUT.PUT_LINE('검색 결과 '||'&maker1'||' ['
	  											||cnt_maker1||']대, '||'&maker2'||' ['
	  											||cnt_maker2||']대');

	ELSE
		DBMS_OUTPUT.PUT_LINE('&maker1'||' 또는 '||'&maker2'||' 에 해당하는 모델이 없습니다.');
	END IF;                                                    
